-- ============================================================
-- Update search_nearby_items and get_listing_detail so group-owned items
-- fall back to groups.coordinates / groups.area_name when no
-- pickup_location_id is set. Also extends search_nearby_items to return
-- area_name directly (it can now come from two tables).
-- ============================================================

DROP FUNCTION IF EXISTS public.search_nearby_items(text, float, float, int, item_category[], item_condition[], item_status, int, int);
...
BEGIN
  ...
  SELECT
    ...
    i.updated_at,
    CASE
      WHEN lat IS NOT NULL AND lng IS NOT NULL AND COALESCE(sl.coordinates, g.coordinates) IS NOT NULL
      THEN ST_Distance(
        COALESCE(sl.coordinates, g.coordinates),
        ST_SetSRID(ST_MakePoint(lng, lat), 4326)::geography
      )
      ELSE NULL
    END AS distance_meters,
    COALESCE(sl.area_name, g.area_name) AS area_name
  FROM items i
  ...
  WHERE
    ...
    AND (
      ...
      OR COALESCE(sl.coordinates, g.coordinates) IS NULL
      OR ST_DWithin(
        COALESCE(sl.coordinates, g.coordinates),
        ST_SetSRID(ST_MakePoint(lng, lat), 4326)::geography,
        max_distance_meters
...
-- get_listing_detail: same fallback for distance and area_name on group items.
...
BEGIN
  ...
  SELECT
    ...
    i.updated_at,
    CASE
      WHEN COALESCE(sl.coordinates, g.coordinates) IS NOT NULL
        AND caller_loc.coordinates IS NOT NULL
      THEN ST_Distance(COALESCE(sl.coordinates, g.coordinates), caller_loc.coordinates)
      ELSE NULL
    END AS distance_meters,
    COALESCE(sl.area_name, g.area_name) AS area_name,
