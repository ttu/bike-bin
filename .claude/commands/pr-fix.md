...
2. Check CI/CD status first:
   ...
   ```bash
   gh pr checks {pr_number} --repo ttu/bike-bin
   ```
...
3. Fetch CodeRabbit review comments from the PR:
   ...
   gh api --paginate "repos/ttu/bike-bin/pulls/{pr_number}/comments" \
   ...
   gh api --paginate "repos/ttu/bike-bin/issues/{pr_number}/comments" \
...
5. Check SonarCloud issues:
   ...
   # https://sonarcloud.io/api/issues/search?componentKeys=ttu_bike-bin&pullRequest={pr_number}&resolved=false
   ```
   ...
   - Or visit web UI: `https://sonarcloud.io/project/issues?id=ttu_bike-bin&pullRequest={pr_number}`
...
7. Check human reviewer comments:
   ...
   gh api "repos/ttu/bike-bin/issues/{pr_number}/comments" \
...
9. After fixing all issues:
   - Run `npm run validate` to check format, lint, types, tests, and build
...
1. Get comment IDs with context:
   ...
   ```bash
   gh api "repos/ttu/bike-bin/pulls/{pr_number}/comments" \
...
2. Reply to a comment explaining why it won't be fixed:
   ...
   ```bash
   gh api "repos/ttu/bike-bin/pulls/{pr_number}/comments/{comment_id}/replies" \
