...
3. **GitHub** — see [deployments.md](deployments.md) §7 for the full list. In short:
   ...
   - On **`preview`**, add variable **`SUPABASE_STAGING_PROJECT_REF`** (staging parent project ref) so CI can resolve each PR’s Supabase preview branch URL, anon key, and (when returned by the Management API) **service role** via **`api-keys?reveal=true`**. If a preview branch does not exist for the PR, CI uses optional secrets **`STAGING_EXPO_PUBLIC_SUPABASE_URL`** / **`STAGING_EXPO_PUBLIC_SUPABASE_ANON_KEY`**, or the **`preview`** **`EXPO_PUBLIC_*`** secrets (often the same as staging) as fallback. Add secret **`SUPABASE_DB_PASSWORD`** on **`preview`** so CI can apply **`supabase/seed.sql`** to a resolved preview branch (`db:seed:remote`); use the branch project’s database password from the Supabase dashboard when it differs from staging. For **Storage seed images** on a resolved preview branch, CI prefers the **service role from the API response**; if your token does not expose it, add optional secret **`SUPABASE_SERVICE_ROLE_KEY`** on **`preview`** (e.g. copy the preview project’s service role from the dashboard for that branch).
   - In **`staging`** and **`production`**, add variable **`SUPABASE_PROJECT_REF`** and optional secret **`SUPABASE_DB_PASSWORD`** for Supabase CLI deploys. On **`staging`**, add secret **`SUPABASE_SERVICE_ROLE_KEY`** for the staging project so **Deploy Supabase staging** can run the image upload step after **`seed.sql`**.
