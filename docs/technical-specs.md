...
| Hosting        | **Supabase** + **Expo EAS** + **EAS Hosting** (web)                 | Backend on Supabase; native builds via EAS; web app exported with `expo export` and deployed to [EAS Hosting](https://docs.expo.dev/eas/hosting/introduction/) on merge to `main` (see `.github/workflows/deploy-web.yml`). |
...
| CI             | **GitHub Actions**                                                  | Lint, type-check, test, coverage, web export, marketing build; **Deploy web** to EAS Hosting after CI passes on `main`. Native store builds via EAS Build remain manual or separate automation.                             |
...
| ------------- | --------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Web (app)** | `expo export --platform web` → **EAS Hosting** (production URL from Expo dashboard / custom domain) | **Continuous** — after CI succeeds on `main`, GitHub Actions runs `eas deploy --prod` (see `deploy-web.yml`). Instant availability, no store review. |
