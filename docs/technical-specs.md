...
| Hosting        | **Supabase** + **Expo EAS** + **EAS Hosting** (web)                 | Backend on Supabase; native builds via EAS; web exported to [EAS Hosting](https://docs.expo.dev/eas/hosting/introduction/) — **preview** on each same-repo PR via `deploy-web-preview` in `ci.yml`, **production** on push to `main` via `.github/workflows/deploy-web.yml`. |
...
| CI             | **GitHub Actions**                                                  | Lint, type-check, test, coverage, web export, marketing build; **deploy-web-preview** (EAS preview on PRs to `main`); **Deploy web** workflow (`eas deploy --prod` after CI on `main`). Native store builds via EAS Build remain manual or separate automation.              |
...
| ------------- | --------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Web (app)** | `expo export --platform web` → **EAS Hosting** (production URL from Expo dashboard / custom domain) | **Continuous** — PRs: preview deploy from `ci.yml` after `build`; `main`: `eas deploy --prod` via `deploy-web.yml`. Instant availability on web, no store review. |
