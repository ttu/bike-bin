...
- **`.github/workflows/close-eas-web-preview.yml`** — on **closed** PRs to `main` (same-repo only), deletes the EAS Hosting preview deployment for that PR via `eas deploy:delete` (uses the deployment id embedded in the preview PR comment).
- **`.github/workflows/deploy-web-staging.yml`** — after **CI** completes successfully on **push to `main`**, runs **`eas deploy --alias staging`** to EAS Hosting (or **workflow_dispatch** on `main`). **`.github/workflows/deploy-web-production.yml`** — on **`v*`** tag push, runs **`eas deploy --prod`**. PR previews live in **ci.yml** (`deploy-web-preview`). Requires `eas.json` (from `eas init`) and repository secrets; see [development.md](development.md) (Automated deploy).
- **`.github/workflows/deploy-supabase-staging.yml`** / **`deploy-supabase-production.yml`** — **`supabase db push`** + Edge Functions; jobs use GitHub Environments **`staging`** / **`production`** (`SUPABASE_PROJECT_REF`, `SUPABASE_DB_PASSWORD`) and repo **`SUPABASE_ACCESS_TOKEN`**. **Web** deploy jobs use the same environments for **`EXPO_PUBLIC_*`**. See [deployments.md](deployments.md) §7.
