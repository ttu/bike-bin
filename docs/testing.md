...
| `npm run test:e2e`          | Playwright — run via this script (specs under `e2e/`; uses the local `playwright` CLI from `package.json`)                                                                                                      |
| `npm run capture:media`     | Playwright — `playwright.media.config.ts`; marketing / App Store PNGs (raw + framed), WebM, optional GIF (`ffmpeg`) under `sites/marketing/public/captures/` (same Supabase + `.env.local` expectations as E2E) |
...
**Deployed web (CI / manual):** Set **`PLAYWRIGHT_BASE_URL`** to the HTTPS URL of the hosted bundle. **`npm run test:e2e:remote-smoke`** runs only `e2e/smoke.spec.ts` — unauthenticated flows, **no** `global-setup` (no local `psql` truncate/seed); use this against **shared staging** where the DB is not reset per deploy. **`npm run test:e2e:remote-full`** runs the same spec groups as local E2E but still **without** `global-setup`; use only with a disposable or seeded preview database. Spec groupings: `e2e/playwright-e2e-blocks.ts`. On PR CI, `scripts/resolve-supabase-pr-preview-env.ts` writes **`.bike-bin-ci-supabase.json`** (`e2eRemoteSuite`: **`full`** when a Supabase **preview branch** exists for the PR, **`smoke`** when CI falls back to staging / static preview secrets — e.g. no `supabase/` changes so Branching did not create a preview DB).
...
| `scripts/resolve-supabase-pr-preview-env.ts`       | CI: resolve PR preview Supabase URL/key; writes `.bike-bin-ci-supabase.json` for remote E2E default suite      |
