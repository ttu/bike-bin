...
describe('useMyBikeLimit', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    mockRpc.mockResolvedValue({ data: 50, error: null });
  });

  it('returns the bike limit from rpc', async () => {
    const { result } = renderHook(() => useMyBikeLimit(), {
    ...
    expect(mockRpc).toHaveBeenCalledWith('get_my_bike_limit');
  });
